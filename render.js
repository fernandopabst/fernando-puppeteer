...
function createTriGridForScene(scene) {
  ...
  const minX = evenFloor(
    voxelCoordToNWTriCoord(vec3Create(0, scene.size.y - 1, 0)).x
  );
  const maxX = evenCeil(
    voxelCoordToNWTriCoord(vec3Create(scene.size.x - 1, 0, 0)).x + 2
  );
  const minY = evenFloor(
    voxelCoordToNWTriCoord(
      vec3Create(scene.size.x - 1, scene.size.y - 1, scene.size.z - 1)
    ).y
  );
...
function* generateEdgesNESW(triGrid) {
  ...
  for (let y = 0; y <= triGrid.size.y; y++) {
    // intentionally go one row past
    for (let x = y & 1; x < triGrid.size.x; x += 2) {
      const tBelow =
        y === triGrid.size.y ? null : triGrid.arr[x + triGrid.size.x * y];
...
function* generateEdgesVertical(triGrid) {
  ...
  for (let y = 0; y < triGrid.size.y; y++) {
    for (let x = y & 1 ? 0 : 1; x <= triGrid.size.x; x += 2) {
      // intentionally go one col past
      const tRight =
        x === triGrid.size.x ? null : triGrid.arr[x + triGrid.size.x * y];
...
function scaleIntegerSegments(segments) {
  return segments.map(([a, b]) => [
    { x: VERT_COORD_SCALE_X * a.x, y: VERT_COORD_SCALE_Y * a.y },
    { x: VERT_COORD_SCALE_X * b.x, y: VERT_COORD_SCALE_Y * b.y },
  ]);
}

function segmentGroupsToSVG(triGrid, segmentGroups, svgOpts) {
  const padFrac =
    svgOpts && svgOpts.padFrac !== undefined ? svgOpts.padFrac : 0;
  const splitPaths =
    svgOpts && svgOpts.splitPaths !== undefined ? svgOpts.splitPaths : false;

  const combinedSegments = [].concat(
    combineSegments(segmentGroups.nesw),
    combineSegments(segmentGroups.nwse),
    combineSegments(segmentGroups.vertical)
  );
  ...
  if (splitPaths) {
    for (const seg of scaledSegments) {
      pathPieces.push(
        `<path d="M ${seg[0].x} ${seg[0].y} L ${seg[1].x} ${seg[1].y}" />`
      );
    }
  } else {
...
function segmentGroupsToFlatUncombinedScaled(segmentGroups) {
  const flatSegments = [].concat(
    segmentGroups.nesw,
    segmentGroups.nwse,
    segmentGroups.vertical
  );
