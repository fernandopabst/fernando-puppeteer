...
function randVec(scene) {
  return vec3Create(
    getRandomInt(0, scene.size.x + 1),
    getRandomInt(0, scene.size.y + 1),
    getRandomInt(0, scene.size.z + 1)
  );
}
...
function worm(scene, startVec, size, moveRepeat, kind) {
  ...
  while (vecInsideScene(scene, pos)) {
    const minCorner = {
      x: Math.floor(pos.x - 0.5 * size),
      y: Math.floor(pos.y - 0.5 * size),
      z: Math.floor(pos.z - 0.5 * size),
    };
